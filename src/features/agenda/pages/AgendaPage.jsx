...
export default function AgendaPage() {
  ...
  // stores
  const { listByDate, addSession, updateSession, removeSession } = useSessionsStore()
  const sessions = useSessionsStore((s) => s.sessions) // para refrescar la vista cuando cambie
  const projects = useProjectsStore((s) => s.projects)

  // sesiones del día
  const daySessions = useMemo(() => listByDate(date), [date, sessions, listByDate])

  // opciones de horas (una fila por hora)
  const slots = useMemo(() => {
    const arr = []
  ...
  // helpers
  const pname = (session) => projects.find((p) => p.id === session.projectId)?.name || '(Sin proyecto)'
  ...
  const overlaps = (aStart, aEnd, ignoreId) => {
    ...
    return daySessions.some((s) => {
      ...
      const e1 = new Date(s.end).getTime()
      return Math.max(start, s1) < Math.min(end, e1)
    })
  }

  const outsideHours = (aStart, aEnd) => {
    const min = new Date(`${date}T${String(START_HOUR).padStart(2, '0')}:00`).getTime()
    const max = new Date(`${date}T${String(END_HOUR).padStart(2, '0')}:00`).getTime()
    const st = new Date(aStart).getTime()
    const en = new Date(aEnd).getTime()
    return st < min || en > max
  }

  const createOrUpdate = (payload) => {
    if (outsideHours(payload.start, payload.end)) {
      alert('La sesión debe estar entre 10:00 y 21:00')
      return
    }
    if (overlaps(payload.start, payload.end, editing?.id)) {
      alert('Se solapa con otra sesión del día')
      return
    }
    editing ? updateSession(editing.id, payload) : addSession(payload)
  }

  // para posicionar bloques en la grilla
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 }}>
        ...
        />
        <button style={btnPri} onClick={() => { setEditing(null); setOpen(true) }}>
          + Nueva sesión
        </button>
      </div>
      ...
      >
        ...
        <div style={{ borderRight: '1px solid #eee', background: '#fafafa' }}>
          {slots.map((s) => (
            <div
              key={s.value}
              style={{ height: 56, padding: '8px 10px', borderBottom: '1px dashed #f0f0f0', color: '#6b7280' }}
            >
              {s.label}
            </div>
          ))}
        ...
        <div style={{ position: 'relative' }}>
          ...
          {daySessions.map((s) => {
            ...
            return (
              ...
              >
                <div style={{ fontWeight: 600 }}>{pname(s)}</div>
                <div style={{ fontSize: 12, color: '#374151' }}>
                  {fmt(s.start)} — {fmt(s.end)} · {s.owner || '—'}
                </div>
                <div style={{ marginTop: 6, display: 'flex', gap: 8 }}>
                  <button style={btnSec} onClick={() => { setEditing(s); setOpen(true) }}>
                    Editar
                  </button>
                  <button
                    style={btnDanger}
                    onClick={() => {
                      if (confirm('¿Eliminar sesión?')) removeSession(s.id)
                    }}
                  >
                    Eliminar
                  </button>
